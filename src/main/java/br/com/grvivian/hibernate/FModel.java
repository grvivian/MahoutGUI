 ...
 *
 * <p>
 * {@code userID,itemID[,preference[,timestamp]]}</p>
 *
 * <p>
 ...
 * IDs and item IDs are read parsed as {@code long}s. The timestamp, if present,
 * is assumed to be parseable as a {@code long}, though this can be overridden
 * via {@link #readTimestampFromString(String)}. The preference value may be
 * empty, to indicate "no preference value", but cannot be empty. That is, this
 * is legal:
 * </p>
 *
 * <p>
 * {@code 123,456,,129050099059}</p>
 *
 * <p>
 * But this isn't:</p>
 *
 ...
 *
 * <p>
 * It is also acceptable for the lines to contain additional fields. Fields
 * beyond the third will be ignored. An empty line, or one that begins with '#'
 ...
 */
public class FModel extends AbstractDataModel {

  private static final Logger log = LoggerFactory.getLogger(FModel.class);
  ...
  /**
   * @param delimiterRegex If your data file don't use '\t' or ',' as
   * delimiters, you can specify user own using regex pattern.
   * @throws IOException
   */
  public FModel(File dataFile, boolean transpose, long minReloadIntervalMS, String delimiterRegex) throws IOException {
   ...
   * any file whose name starts the same way as the data file (up to first
   * period) but isn't the data file itself. For example, if the data file is
   ...
   * {@link FastByIDMap} data structure which maps user IDs to preferences. This
   * assumes that each line of the input file corresponds to one preference.
   * After reading a line and determining which user and item the preference
   * pertains to, the method should look to see if the data contains a mapping
   * for the user ID already, and if not, add an empty data structure of
   ...
   * @param fromPriorData an implementation detail -- if true, data will map IDs
   * to {@link PreferenceArray} since the framework is attempting to read and
   * update raw data that is already in memory. Otherwise it maps to
   * {@link Collection}s of {@link Preference}s, since it's reading fresh data.
   * Subclasses must be prepared to handle this wrinkle.
   */
  ...
  /**
   * Subclasses may wish to override this to change how time values in the input
   * file are parsed. By default they are expected to be numeric, expressing a
